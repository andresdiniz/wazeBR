...
    header("Location: /login_page.php"); // Redireciona para a página de login
    exit();
}
...
    
    // Prepara dados para a página de erro 404
    $errorData = $dados;
    $errorData['errorCode'] = 404;
    $errorData['errorTitle'] = 'Página Não Encontrada';
    $errorData['errorMessage'] = 'O recurso solicitado não pôde ser encontrado.';
    $errorData['errorDescription'] = 'Verifique o endereço e tente novamente.';
    $errorData['pagina_retorno'] = '/';

    // Verifica se o template 404.twig existe, caso contrário, usa o error.twig ou mostra uma mensagem simples
    if ($loader->exists('404.twig')) {
         echo $twig->render('404.twig', $errorData);
    } elseif ($loader->exists('error.twig')) {
        // Se não tiver 404, usa o template genérico de erro
...
    }
    ...
    // --- NOVO TRATAMENTO DINÂMICO DE ERROS ---
    
    // 1. Determina o código de erro HTTP
    $code = $e->getCode();
    
    // Se o código da exceção não for um status HTTP válido (4xx ou 5xx),
    // ou se for um código de erro genérico (como 0 ou 1), forçamos o 500.
    ...
    // Lida com log e email, como antes
    ...
    $errorData['errorCode'] = $code; // AGORA O CÓDIGO É DINÂMICO

    // Mapeamento para Títulos de Erro Padrão
    $titleMap = [
    ...
    // Define o Título (usa o mapeamento, ou um default)
    $errorData['errorTitle'] = $titleMap[$code] ?? 'Erro Desconhecido';

    // Define a Mensagem (mostra ou esconde detalhes baseado no código e no modo DEBUG)
    if ($code >= 400 && $code < 500) {
        // Para erros de cliente (4xx), a mensagem da exceção é geralmente amigável
        $errorData['errorMessage'] = $e->getMessage() ?: 'Verifique as informações e tente novamente.';
        $errorData['errorDescription'] = 'Verifique se as informações estão corretas.';
    } else {
        // Para 500, mostra detalhes apenas se estiver em modo DEBUG
        $errorData['errorMessage'] = isset($_ENV['DEBUG']) && $_ENV['DEBUG'] === 'true' 
            ? $e->getMessage() 
            : 'Ocorreu um erro inesperado no servidor. Tente novamente mais tarde.'; 
            
        $errorData['errorDescription'] = 'Detalhes do erro registrados para análise.';
    }
    ...
    if ($loader->exists('error.twig')) {
        ...
        echo "<h1>{$code} {$errorData['errorTitle']}</h1><p>{$errorData['errorMessage']}</p>";
        // Em debug, mostrar o erro detalhado se o template de erro não existir
        if (isset($_ENV['DEBUG']) && $_ENV['DEBUG'] === 'true' && $code === 500) {
