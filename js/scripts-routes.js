document.addEventListener('DOMContentLoaded', () => {
    ...
    let routeLayer = null; // Camada da rota no mapa Leaflet
    let heatmapChartInstance = null; // Instância do chart Highcharts

    // Adiciona ouvintes de evento a todos os botões com a classe 'view-route'
    document.querySelectorAll('.view-route').forEach(button => {
        button.addEventListener('click', async () => {
            // Obtém o ID da rota a partir do atributo data-route-id do botão
            const routeId = button.dataset.routeId;

            // Seleciona os elementos dentro do modal para atualizar ou manipular
            ...
            // Supondo que haja um container específico para os insights
            const insightsContainer = mapModal.querySelector('#insightsContainer');

            // Limpa o conteúdo anterior e destrói instâncias antigas para evitar vazamento de memória
            modalTitle.textContent = 'Carregando...';

            // Destrói a instância anterior do mapa Leaflet, se existir
            if (mapInstance) {
                mapInstance.remove(); // Método Leaflet para remover o mapa completamente
                mapInstance = null; // Reseta a referência
            }
            // Destrói a instância anterior do chart Highcharts, se existir
            if (heatmapChartInstance) {
                heatmapChartInstance.destroy(); // Método Highcharts para destruir o chart
                heatmapChartInstance = null; // Reseta a referência
            }

            // Limpa os containers HTML. Importante APÓS destruir as instâncias, se aplicável.
            // Limpar o mapContainer.innerHTML pode ser necessário se Leaflet não limpar tudo,
            // ou se você quiser garantir que o div está vazio para recriar o mapa nele.
             if (mapContainer) mapContainer.innerHTML = '';
             if (heatmapChartContainer) heatmapChartContainer.innerHTML = '';
            ...
            try {
                // Faz a requisição assíncrona para buscar os detalhes da rota
                const response = await fetch(`/api.php?action=get_route_details&route_id=${routeId}`);

                // Verifica se a resposta da rede foi bem-sucedida
                if (!response.ok) {
                    // Lança um erro se a resposta HTTP não for OK (ex: 404, 500)
                ...
                // Verifica erros retornados pela API na estrutura do payload JSON
                if (result.error) {
                    // Melhoria: Em vez de alert, exibir em um elemento dedicado no modal para melhor UX
                    console.error('Erro retornado pela API:', result.error);
                    alert('Erro ao buscar detalhes: ' + result.error); // Mantido alert por simplicidade, mas sugere-se melhorar
                    return; // Interrompe a execução se houver erro da API
                }

                // Desestrutura os dados recebidos
                const { route, geometry, historic, heatmap, subroutes } = result.data;

                // Atualiza o título do modal com o nome da rota
                if (modalTitle) modalTitle.textContent = route.name || 'Detalhes da Rota';

                // Chama as funções para renderizar os diferentes componentes
                if (mapContainer) renderMap(mapContainer.id, geometry);
                if (heatmapChartContainer) renderHeatmap(heatmapChartContainer.id, heatmap, route);
                if (insightsContainer) renderInsights(insightsContainer, route, geometry, heatmap);

            } catch (err) {
                // Captura erros durante o fetch ou processamento
                console.error('Erro ao carregar rota:', err);
                 // Melhoria: Exibir em um elemento dedicado no modal para melhor UX
                alert('Erro ao carregar rota. Veja o console para mais detalhes.'); // Mantido alert por simplicidade, mas sugere-se melhorar
            } finally {
                // Garante que o indicador de carregamento seja ocultado no final,
    ...
    /**
     * Renderiza o mapa Leaflet com a geometria da rota.
     * @param {string} containerId O ID do elemento HTML onde o mapa será renderizado.
     * @param {Array<Object>} geometry Array de pontos com {x: longitude, y: latitude}.
     */
    ...
    /**
     * Renderiza os insights e estatísticas da rota em um elemento HTML.
     * @param {HTMLElement} containerElement O elemento HTML onde os insights serão renderizados.
     * @param {Object} route Objeto com dados gerais da rota (nome, avg_speed, historic_speed, jam_level).
     * @param {Array<Object>} geometry Array de pontos da geometria (usado para contar irregularidades).
     * @param {Array<Object>} heatmapData Array de objetos com dados de velocidade por hora/dia ({day_of_week, hour, avg_speed}).
     */
    function renderInsights(containerElement, route, geometry, heatmapData) {
        ...
        // Monta o HTML para a seção de insights.
        // Este é um exemplo básico. Recomenda-se usar um template engine ou criar elementos dinamicamente
        // de forma mais robusta para layouts complexos.
        const insightsHTML = `
                ...
                <div class="col-md-6">
                    ...
                    <p>
                        ...
                        <span class="${speedVariation >= 0 ? 'text-success' : 'text-danger'}">
                         ...
                         ${/* Exemplo de barra de progresso visualizando a diferença de velocidade */''}
                         <div class="progress" style="height: 5px; margin-top: 5px;">
                            <div class="progress-bar ${speedVariation >= 0 ? 'bg-success' : 'bg-danger'}"
                                role="progressbar" style="width: ${Math.min(Math.abs(speedVariation), 100)}%;" // Largura baseada na magnitude da variação (limita a 100%)
                 ...
                 <div class="col-md-6">
                    ...
                    <h5>Distribuição de Velocidade (Dados Históricos Agregados)</h5>
                    ...
                    ${/* Container para o sparkline da distribuição de velocidades */''}
                     <p><strong>Variação das Velocidades Históricas:</strong> <span class="speed-sparkline" data-values="${speeds.join(',')}"></span></p>

                </div>
            </div>
        `;

        // Adiciona o HTML gerado ao container de insights no modal
        containerElement.innerHTML = insightsHTML;

        // --- Inicializa componentes que dependem do conteúdo recém-adicionado ---

        // Inicializa Bootstrap Tooltips dentro do container de insights
        // O trigger 'hover' é o padrão para tooltips via data-bs-toggle, mas pode ser explicitado
        containerElement.querySelectorAll('[data-bs-toggle="tooltip"]').forEach(el => {
             // Opcional: Destruir instância antiga para evitar duplicidade, se o Bootstrap não lidar automaticamente
             const tooltipInstance = bootstrap.Tooltip.getInstance(el);
             if (tooltipInstance) {
                 tooltipInstance.dispose();
             }
             new bootstrap.Tooltip(el); // Inicializa uma nova instância
        });

        // Inicializa Sparklines dentro do container de insights
         containerElement.querySelectorAll('.sparkline').forEach(spark => {
             // Obtém os valores do atributo data-values, converte para números e filtra NaNs
             const values = spark.dataset.values.split(',').map(Number).filter(v => !isNaN(v));

             // Cria o sparkline apenas se houver dados suficientes (pelo menos 2 pontos são geralmente necessários para uma linha)
             if (values.length > 1) {
                  new Sparkline(spark, {
                     width: 120, // Largura ajustável
                     height: 30, // Altura ajustável
                     lineColor: '#4e73df', // Cor da linha (ex: azul primário)
                     fillColor: '#d1d3e2', // Cor de preenchimento (ex: cinza claro)
                     spotColor: null, // Remove ponto no último valor padrão
                     minSpotColor: '#f6c23e', // Cor para o ponto mínimo (amarelo)
                     maxSpotColor: '#1cc88a', // Cor para o ponto máximo (verde)
                     highlightSpotColor: '#e74a3b', // Cor do ponto ao passar o mouse (vermelho)
                     highlightLineColor: '#212529', // Cor da linha ao passar o mouse (cinza escuro)
                     spotRadius: 2 // Raio dos pontos
                 });
             } else {
                 // Se não houver dados suficientes, exibe um texto em vez do gráfico
                 spark.innerText = values.length === 1 ? `(${values[0].toFixed(1)} km/h)` : '(Sem dados de velocidade)';
             }
         });
    }

    /**
     * Renderiza o gráfico de heatmap de velocidade por hora e dia da semana usando Highcharts.
     * @param {string} containerId O ID do elemento HTML onde o gráfico será renderizado.
     * @param {Array<Object>} heatmapData Array de objetos com {day_of_week, hour, avg_speed}.
     * @param {Object} route Objeto com dados gerais da rota (pode ser usado para título, etc., se necessário).
     */
