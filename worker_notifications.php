    ...
    foreach ($pendentes as $envio) {
        ...
        try {
            ...
            if (!empty($envio['email'])) {
                ...
                echo "Enviando notificação WhatsApp para o número: {$envio['phone']} com UUID do alerta: {$envio['id']}" . PHP_EOL;
                $status = enviarNotificacaoWhatsApp($pdo, $deviceToken, $authToken, $envio['phone'], $envio['id']);
